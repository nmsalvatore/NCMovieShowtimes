...
async function getSchedule() {
    ...
    const response = await fetch(
        "https://www.prime-cinemas.com/api/gatsby-source-boxofficeapi/schedule",
        {
            credentials: "include",
            referrer:
                "https://www.prime-cinemas.com/showtimes/g0240-prime-cinemas-del-oro/",
            body: `{"circuit":null,"theaters":[{"id":"G0240","timeZone":"America/Los_Angeles"}],"movieIds":${movieIdsAsString},"from":"${startDate}","to":"${endDate}","nin":[],"sin":[],"websiteId":"V2Vic2l0ZU1hbmFnZXJXZWJzaXRlOjZkZmVkZWQzLTc5MDUtNGY3ZS05M2JiLWFkOWZjOTk0ZDFhMA=="}`,
            method: "POST",
            mode: "cors",
        },
    );
    ...
    const dates = await data.G0240.schedule;
    return dates;
}

async function getDelOroShowings() {
    ...
    for (const id in schedule) {
        ...
        for (const movieDate in movieShowtimesByDate) {
            ...
            for (const showtimeData of daysMovieShowtimeData) {
                ...
                const venue = "Prime Cinemas Del Oro";
                const address = "165 Mill Street, Grass Valley, CA 95945";
...
export default getDelOroShowings;
